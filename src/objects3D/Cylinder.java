	...
	public void DrawCylinder(float radius, float height, int nSegments ) 
	{
		...
		{ /* a loop around circumference of a tube */
			...
			Point4f vertices[] = new Point4f[]{
					new Point4f(x2, y2, height, 0),new Point4f(x1, y1, height, 0),new Point4f(0, 0, height, 0),
					new Point4f(x1, y1, 0, 0),new Point4f(x2, y2, 0, 0),new Point4f(0, 0, 0, 0)
					};
			//this is the points that draw the round surface
			Point4f points[] = {new Point4f(x1, y1, 0, 0),new Point4f(x2, y2, height, 0),new Point4f(x1, y1, height, 0),
					new Point4f(x1, y1, 0, 0),new Point4f(x2, y2, 0, 0),new Point4f(x2, y2, height, 0)
			};
			
			//draw the cylinder
			for (int j = 0; j < points.length; j++) {
				GL11.glNormal3f(points[j].x, points[j].y, 0f);
			...
			v = vertices[3].MinusPoint(vertices[5]); 
			w = vertices[4].MinusPoint(vertices[5]);
