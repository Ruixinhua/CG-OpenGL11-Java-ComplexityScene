...
public class Rocket {
    ...
    float radius;
    float height;
    float groundUp; // the distance far from the ground
    float speed;
    Cylinder cylinder;
    Cone cone;
    Sphere sphere;
...
//            GL11.glEnable(GL11.GL_LIGHT4);
            ...
            GL11.glRotatef(90,1,0,0);
            cone.DrawCone(radius,height,32, 0.5f);
        }
        GL11.glPopMatrix();
    }

    public void drawRocketBody(float radius, float height){
        GL11.glPushMatrix();
        {
            GL11.glColor3f(red[0], red[1], red[2]);
            GL11.glMaterial(GL11.GL_FRONT, GL11.GL_AMBIENT_AND_DIFFUSE, Utils.ConvertForGL(red));
            cylinder.DrawCylinder((radius * 3) / 5, height, 32);
